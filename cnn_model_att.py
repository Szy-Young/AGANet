...
class CNN_Model:
    ...
    def net(self, skt_input, name='CNN_action', norm_pos=False, norm_scale=False):
        ...
        with tf.variable_scope(name):
            ...
            # --Spatial attention
            x_s = tf.contrib.layers.batch_norm(x, decay=0.9, epsilon=1e-5, is_training=self.training, scope='spatial_attention_bn1')
            s_att = tf.layers.conv2d(x_s, 1, (5, 1), padding='SAME', activation=tf.nn.sigmoid, trainable=self.training, name='spatial_attention_conv1')
            s_att = tf.tile(s_att, [1, 1, 1, 8])
            x += tf.multiply(s_att, x)
            ...
            # Import attention mechanism
            # --Channel attention
            x_c = tf.contrib.layers.batch_norm(x, decay=0.9, epsilon=1e-5, is_training=self.training, scope='channel_attention_bn1')
            x_c = tf.layers.conv2d(x_c, 64, (5, 1), strides=(2, 1), padding='SAME', activation=tf.nn.relu, trainable=self.training, name='channel_attention_conv1')
            x_c = tf.contrib.layers.batch_norm(x_c, decay=0.9, epsilon=1e-5, is_training=self.training, scope='channel_attention_bn2')
            x_c = tf.layers.conv2d(x_c, 64, (5, 1), padding='SAME', activation=tf.nn.relu, trainable=self.training, name='channel_attention_conv2')
            c_max = tf.reduce_max(x_c, axis=1, keepdims=True)
            c_avg = tf.reduce_mean(x_c, axis=1, keepdims=True)
            c_sq = tf.concat([c_max, c_avg], axis=-1)
            c_att = tf.contrib.layers.batch_norm(c_sq, decay=0.9, epsilon=1e-5, is_training=self.training, scope='channel_attention_bn3')
            c_att = tf.layers.conv2d(c_att, 64, (1, 1), padding='SAME', activation=tf.nn.sigmoid, trainable=self.training, name='channel_attention_conv3')
            c_att = tf.tile(c_att, [1, self.seq_len//4, 1, 1])
            ...
            """
            # Encode the spatial patterns
            x = tf.layers.conv2d(skt_input, 8, (1, 5), strides=(1, 2), padding='SAME', trainable=self.training, name='spatial_conv1')
            x = tf.contrib.layers.batch_norm(x, decay=0.9, epsilon=1e-5, activation_fn=tf.nn.relu, is_training=self.training, scope='spatial_bn1')
            x = tf.layers.conv2d(x, 32, (1, 5), padding='VALID', trainable=self.training, name='spatial_conv2')
            x = tf.contrib.layers.batch_norm(x, decay=0.9, epsilon=1e-5, activation_fn=tf.nn.relu, is_training=self.training, scope='spatial_bn2')
            # Encode the temporal patterns
            x = tf.layers.conv2d(x, 32, (5, 1), strides=(2, 1), padding='SAME', trainable=self.training, name='temporal_conv1')
            x = tf.contrib.layers.batch_norm(x, decay=0.9, epsilon=1e-5, activation_fn=tf.nn.relu, is_training=self.training, scope='temporal_bn1')
            x = tf.layers.conv2d(x, 64, (5, 1), strides=(2, 1), padding='SAME', trainable=self.training, name='temporal_conv2')
            x = tf.contrib.layers.batch_norm(x, decay=0.9, epsilon=1e-5, activation_fn=tf.nn.relu, is_training=self.training, scope='temporal_bn2')
            x = tf.layers.conv2d(x, 64, (5, 1), padding='SAME', trainable=self.training, name='temporal_conv3')
            x = tf.contrib.layers.batch_norm(x, decay=0.9, epsilon=1e-5, activation_fn=tf.nn.relu, is_training=self.training, scope='temporal_bn3')
            x = tf.layers.conv2d(x, 64, (5, 1), padding='SAME', trainable=self.training, name='temporal_conv4')
            x = tf.contrib.layers.batch_norm(x, decay=0.9, epsilon=1e-5, activation_fn=tf.nn.relu, is_training=self.training, scope='temporal_bn4')
            # Upsample the temporal dimension
            x = tf.image.resize_images(x, size=(self.seq_len, 1))
            # Reduce the spatial dimension
            x = tf.squeeze(x, axis=2)
